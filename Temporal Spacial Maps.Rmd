...
geo_data_prec_incidence_yearframe <- left_join(x = , y = geo_data_prec, by = c("Districts_names"))
geo_data_prec_incidence_yearframe_sf <- st_as_sf(geo_data_prec_incidence)
...
```{r}
years2006_2002_prec <- c(``,``,``)
years2006_2002_incidence <- c(``,``,``)

for (yearframe_prec in years2006_2020_prec) {
  for (yearframe_incidence in years2006_2020_incidence){
  prec_incidence_plot <- ggplot() +
  geom_sf(data = geo_data_prec_incidence_sf, aes(fill = !!as.name(yearframe_prec)) +
  ggtitle(label = "Precipitation and Incidence", subtitle = yearframe_prec) +
  geom_point(data = geo_data_prec_incidence_sf, aes(x = Longtitude, y = Latitude, size = !!as.name(yearframe_incidence), color = !!as.name(yearframe_incidence)), alpha = 0.95) +
