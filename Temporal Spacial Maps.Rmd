...
for (year in years) {
  pop_plot <- ggplot(data = geo_data_pop) +
    geom_sf(aes(fill = !!as.name(year)), alpha = 0.95) +
    ggtitle(label = "Population Data", subtitle = year) +
    labs(x = "Latitude", y = "Longitude") +
    scale_fill_viridis(option = "plasma", direction = -1) +
    geom_sf_text(aes(label = Districts_names), size = 1, hjust = 1)
...
for (Month in Month2020) {
  prec_plot <- ggplot(data = geo_data_prec) +
    geom_sf(aes(geometry = geometry, fill = !!as.name(Month), alpha = 0.95, geometry = geo_data_prec$geometry) +
...
for (Month in Month2020) {
  prec_plot <- ggplot(data = geo_data_prec) +
    geom_sf(aes(fill = !!as.name(Month)), alpha = 0.95) +
    ggtitle(label = "Precipitation Data", subtitle = Month) +
    labs(x = "Latitude", y = "Longitude") +
    scale_fill_viridis(option = "plasma", direction = -1) +
    geom_sf_text(aes(label = Districts_names), size = 1, hjust = 1)
