...
```{r}
library(readxl)
area <- as.data.frame(read_excel("area_data.xlsx"))
```
...
for (year in 1:15) {
  ...
  for (month in 1:12) {
    ...
    data = cbind(year, month, Incidence_combined[,c(2, (year-1)*12+month+2)], Precipitation[,(year-1)*12+month+4])
    colnames(data) = c("year","month", "provinces", "dengue", "precipitation")
    GAM_data = rbind(GAM_data, data)
...
for (year in 1:15) {
  ...
  if (year == 15) {
    ...
    for (month in 1:11) {
      ...
      data = cbind(year, month, year, month+1, Incidence_combined[,c(2, (year-1)*12+month+3)],
                   Precipitation[,(year-1)*12+month+4])
      
      colnames(data) = c("year for precipitation","month for precipitation","year for incidence",
                           "month for incidence", "provinces", "dengue", "precipitation")
      
      GAM_data_1_Lag = rbind(GAM_data_1_Lag, data)
    }
  }
  # every other year 
  else {
    for (month in 1:12) {
      # exception for last month
      if (month == 12) {
        
        data = cbind(year, month, year+1, 1, Incidence_combined[,c(2, (year-1)*12+month+3)],
                     Precipitation[,(year-1)*12+month+4])
...
```{r}
for (lag in 0:2) {
  assign(paste0("GAM_data_", lag, "_lag"), data.frame(NULL))
  
  for (month in 3:62) {
    data = cbind(province_names[-which(province_names[,1] == "Bungkan"),1],
                 colnames(monsoon_prec)[month],
                 monsoon_prec[ ,month],
                 colnames(get(paste0("monsoon_inc_", lag, "_lag")))[month],
                 get(paste0("monsoon_inc_", lag, "_lag"))[, month])
    
    colnames(data) = c("province","month for precipitation", "precipitation", "month for dengue", "dengue")
    
    assign(paste0("GAM_data_", lag, "_lag"), rbind(get(paste0("GAM_data_", lag, "_lag")), data))
  }
}
```
